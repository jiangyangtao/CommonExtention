...
namespace CommonExtention.Extensions
{
    ...
    public static class HttpPostedFileBaseExtensions
    {
        ...
        #region 将当前 HttpPostedFileBase 用异步方式读取到 DataTable
        /// <summary>
        /// 将当前 <see cref="HttpPostedFileBase"/> 用异步方式读取到 <see cref="DataTable"/>
        /// </summary>
        /// <param name="httpPostedFile">要读取的 <see cref="HttpPostedFileBase"/> 对象</param>
        /// <param name="sheetName">指定读取 Excel 工作薄 sheet 的名称</param>
        /// <param name="firstRowIsColumnName">首行是否为 <see cref="DataColumn.ColumnName"/></param>
        ...
        /// 否则返回从 <see cref="HttpPostedFileBase"/>读取后的 <see cref="DataTable"/> 对象。
        /// </returns>
        public static async Task<DataTable> ReadToDataTableAsync(this HttpPostedFileBase httpPostedFile,
            string sheetName = null,
            bool firstRowIsColumnName = true,
            bool addEmptyRow = false) => await new Excel().ReadHttpPostedFileToDataTableAsync(httpPostedFile, sheetName, firstRowIsColumnName, addEmptyRow);
        #endregion
        ...
        #region 将当前 HttpPostedFileBase 对象用异步方式读取到 ICollection<DataTable>
        /// <summary>
        /// 将当前 <see cref="HttpPostedFileBase"/> 对象用异步方式读取到 <see cref="ICollection{DataTable}"/>
        /// </summary>
        /// <param name="httpPostedFile">要读取的 <see cref="HttpPostedFileBase"/> 对象</param>
        /// <param name="firstRowIsColumnName">首行是否为 <see cref="DataColumn.ColumnName"/></param>
        /// <param name="addEmptyRow">是否添加空行，默认为 false，不添加</param>
        ...
        /// 其中一个 <see cref="DataTable"/> 对应一个 Sheet 工作簿。
        ///</returns>
        public static async Task<ICollection<DataTable>> ReadToTablesAsync(HttpPostedFileBase httpPostedFile,
            bool firstRowIsColumnName = true,
            bool addEmptyRow = false) => await new Excel().ReadHttpPostedFileToTablesAsync(httpPostedFile, firstRowIsColumnName, addEmptyRow);
        #endregion
    }
}
