...
namespace CommonExtention.Extensions
{
    ...
    {
        #region 将当前 Stream 读取到 DataTable
        /// <summary>
        /// 当前 <see cref="Stream"/> 读取到 <see cref="DataTable"/>
        /// </summary>
        ...
        public static DataTable ReadToDataTable(this Stream stream, string sheetName = null, bool firstRowIsColumnName = true, bool addEmptyRow = false)
            => new Excel().ReadStreamToDataTable(stream, sheetName, firstRowIsColumnName, addEmptyRow);
        #endregion

        #region 将当前 Stream 读取到 DataSet
        /// <summary>
        /// 当前 <see cref="Stream"/> 读取到 <see cref="DataSet"/>
        /// </summary>
        /// <param name="stream">要读取的 <see cref="Stream"/> 对象</param>
        /// <param name="firstRowIsColumnName">首行是否为 <see cref="DataColumn.ColumnName"/></param>
        /// <param name="addEmptyRow">是否添加空行，默认为 false，不添加</param>
        ...
        /// </returns>
        public static DataSet ReadToDataSet(this Stream stream, bool firstRowIsColumnName = true, bool addEmptyRow = false)
            => new Excel().ReadStreamToDataSet(stream, firstRowIsColumnName, addEmptyRow);
        #endregion
    }
}
